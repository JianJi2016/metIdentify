...
#'              }
#' 
#'              identification.result <- lapply(objects, function(x){
#'                iden.result <- x@identification.result
#'                match.result <- x@match.result
#'               names(iden.result) <- match.result$MS1.peak.name[match(names(iden.result), match.result$MS2.spectra.name)]
#'               iden.result <- do.call(rbind, iden.result)
#'               Peak.name <- row.names(iden.result)
#'               iden.result <- data.frame(Peak.name, iden.result, stringsAsFactors = FALSE)
#'               rownames(iden.result) <- NULL
#'               iden.result
#'              })
#'              
#'              # database.name <- unlist(lapply(objects, function(x) x@database))
#'              # identification.result <- mapply(function(x,y){
#'              # list(data.frame(x, "Database" = y, stringsAsFactors = FALSE)  )
#'              # },
...
#'              # 
#'              # duplicated.peak.name <- unique(identification.result$Peak.name[duplicated(identification.result$Peak.name)])
#'              # 
#'              # if(length(duplicated.peak.name) > 0){
#'              #   lapply(duplicated.peak.name, function(x){
#'              #     temp.result <- identification.result[which(identification.result$Peak.name == x), ,drop = FALSE]
#'              #   })
#'              # }
#'              
#'              
