...
# 
# rtCor(ref.is.table = "ref.is.table.xlsx", 
...
setGeneric(name = "rtCor4database", 
           def = function(experiment.is.table,
                          database.is.table,
                          database,
             ...
             cor.feature.table <- as.data.frame(database@spectra.info[,c("Lab.ID", "Compound.name", "RT")])
             write.csv(cor.feature.table, file = file.path(path, "cor.feature.table.csv"), 
                       row.names = FALSE)
             
             predicted.rt <- rtCor(ref.is.table = experiment.is.table,
                                   cor.is.table = database.is.table, 
                                   cor.feature.table = "cor.feature.table.csv", 
                                   method = method, poly = poly, degree = degree,
                                   path = path, 
...
setGeneric(name = "rtCor", 
           def = function(
             ...
             ref.is.table <- readTable(file = file.path(path, ref.is.table))
             cor.is.table <- readTable(file = file.path(path, cor.is.table))
             cor.feature.table <- readTable(file = file.path(path, cor.feature.table))
             ...
             cor.feature.table[,3] <- as.numeric(cor.feature.table[,3])
             
             remain.idx <- which(!is.na(ref.is.table[,3]) & !is.na(cor.is.table[,3]))
             ref.is.table <- ref.is.table[remain.idx, , drop = FALSE]
             cor.is.table <- cor.is.table[remain.idx, , drop = FALSE]
             ...
             if(method == "polyline"){
               ...
               result <- bestloess(x = rt.table$cor,
                                   y = rt.table$ref,
                                   span.begin = 0.5,
                                   span.end = 1,
                                   span.step = 0.1,
             ...
             predict.rt[raw.rt < min(rt2)] <- raw.rt[raw.rt < min(rt2)]
             predict.rt[raw.rt > max(rt2)] <- raw.rt[raw.rt > max(rt2)]
             ...
             plot <- ggplot2::ggplot(data = temp.data, ggplot2::aes(x = raw.rt, y = predict.rt)) +
             ...
             ggplot2::ggsave(filename = file.path(path, "Raw.RT vs Predict.RT plot.pdf"), 
                             plot = plot, 
                             width = 8, 
                             height = 6) 
